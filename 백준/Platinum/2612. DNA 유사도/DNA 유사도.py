...
def backTrack(i,j,score,back,res):
    if back[i][j]=='S':
        res[0]=i
        res[1]=j
        return 
        ...
        if back[i][j]=='U':
            backTrack(i-1,j,score,back,res)
    
        elif back[i][j]=='L':
            backTrack(i,j-1,score,back,res)
    
        else:
            backTrack(i-1,j-1,score,back,res)
...
def main():
    ...
    backTrack(maxi,maxj,score_matrix,backtrack_matrix,res)
